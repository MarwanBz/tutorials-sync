...
interface PostFrontmatter {
  title?: string; // Optional for tutorials (derived from H1)
  description: string;
  date?: string; // Optional for tutorials (derived from created or filename)
  slug?: string; // Optional (derived from filename)
  published?: boolean; // Default true
  tags?: string[]; // Optional for tutorials (derived from concepts)
  readTime?: string;
  ...
  concepts?: string; // Comma-separated concepts (like tags)
  source_repo?: string; // Source repository name
  understanding_score?: number; // Quiz score 1-10
  last_quizzed?: string; // Date string when last quizzed
  prerequisites?: string[]; // Array of tutorial slugs
...
function parseMarkdownFile(filePath: string): ParsedPost | null {
  try {
    ...
    // Skip README and other non-tutorial files
    const filename = path.basename(filePath);
    if (filename === "README.md" || filename.startsWith("_")) {
      return null;
    }
    ...
    function normalizeDate(dateStr: string): string {
      // Try DD-MM-YYYY format (tutorial format)
      const dmyMatch = dateStr.match(/^(\d{2})-(\d{2})-(\d{4})$/);
      if (dmyMatch) {
        return `${dmyMatch[3]}-${dmyMatch[2]}-${dmyMatch[1]}`; // YYYY-MM-DD
      }
      // Already in YYYY-MM-DD format or other valid format
      return dateStr;
    }

    // Derive date from created, filename date, or file mtime
    let date = frontmatter.date || frontmatter.created;
    if (date) {
      date = normalizeDate(date);
    } else {
      // Try to extract date from filename (e.g., 2025-12-28-...)
    ...
    if (!frontmatter.description) {
      console.warn(`Skipping ${filename}: missing description in frontmatter`);
      return null;
    }

    // Derive tags from concepts (comma-separated string) if tags not provided
    let tags: string[] = frontmatter.tags || [];
    if (tags.length === 0 && frontmatter.concepts) {
      tags = frontmatter.concepts.split(",").map((c) => c.trim());
    }
    ...
    return {
      ...
      understanding_score: frontmatter.understanding_score ?? undefined,
      last_quizzed: frontmatter.last_quizzed ?? undefined,
