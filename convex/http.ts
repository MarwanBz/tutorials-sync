...
const SITE_URL = process.env.SITE_URL || "https://tutorials-sync.netlify.app";
const SITE_NAME = "Tutorials Sync";
...
http.route({
  ...
  handler: httpAction(async (ctx) => {
    ...
    const response = {
      ...
      url: SITE_URL,
      description:
        "My coding tutorials with quizzes, progress tracking, and spaced repetition.",
...
function generateMetaHtml(content: {
  ...
  const siteUrl = process.env.SITE_URL || "https://tutorials-sync.netlify.app";
  const siteName = "Tutorials Sync";
